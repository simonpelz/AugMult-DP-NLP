...
class GradSampleModuleAugMultBias(AbstractGradSampleModule):
    """
    ...
    def capture_backprops_hook(
        ...
        activations, backprops = self.rearrange_grad_samples( # actvs might be torch.empty(0)
            module=module,
            backprops=backprops,
            loss_reduction=loss_reduction,
            batch_first=batch_first,
        ...
        if not hasattr(module, "activations") or len(module.activations) == 0: #TODO just guessed 
    ...
    def rearrange_grad_samples(
        ...
        *,
        ...
        can return activations = torch.empty(0) if "classifier" not in param name
        ...
        if not hasattr(module, "activations"):
            ...
            for param_name, _ in module.named_parameters():
                if "classifier" in param_name: # TODO FIXME not robust, how identify bias from weight? also fix in setting requires_grad
                    raise ValueError(
                    f"No activations detected for classifier {type(module)},"
                    "activations are not saved for layers where only biases are trained")
        else:
        ...
        if not hasattr(module, "max_batch_len"):
            ...
            module.max_batch_len = backprops.reshape(
                (
                    -1,
                    self.K,
                )
        ...
        if batch_dim != 0:
            if activations.shape != torch.empty(0).shape:
                activations = [
